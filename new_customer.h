...
// OVERLOADED << OPERATOR - a nonmember
                ...
                friend ostream& operator<< (ostream&, const Customer <T>&);
public:
        // Customer(Default)
...
// Returns: ?
                Customer<T>& Customer::operator=(const Customer<T>&);
};
...
{
        n_Capacity = 2;
        new_customer = new T [n_Capacity];
        n_Size = 0;
}

// Parameterized Constructor
template<class T>
Customer<T>::Customer(const T& n)
{
        n_Capacity = n;
        new_customer = new <T> [n_Capacity];
        n_Size = 0;
...
template<class T>
Server<T>::Customer(const Customer<T>& b)
{
                n_Capacity = b.n_Capacity;
                n_Size = b.n_Size;
 new_customer = new T [n_Size];
                for(int i = 0; i < b.n_Size;i++)
                {
                        new_customer[i] = b.new_customer[i];
                }
}

// OPERATOR OVERLOAD
template<class T>
Customer<T>::Customer& operator=(const Customer<T>& b)
{
        // Test for self-copy
        if(this != &b)
        {
...
void Customer<T>::push_back(T& s)
{
        ...
        new_customer[n_Size] = s;
        n_Size++;
}

// Capacity Function
template<class T>
T Customer<T>::capacity() const
{
        return n_Capacity;
}
...
// At Function
template<class T>
T Customer<T>::at(T& n) const
{
        if(n < n_Size)
        {
                return new_customer[n];
        }
...
ostream& operator<<(ostream& out, const Customer<T>& b)
{
        for(int i = 0; i <(b.n_Size); i++)
        {
                out << b.at(i) << " ";
