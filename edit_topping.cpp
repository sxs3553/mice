#include "edit_topping.h"
#include <stdexcept>

namespace Mice {

E_topping::E_topping(std::string name, std::string description, double cost, double\
 price,int amount) : Item(name, description, cost, price), _amount{amount} { }

E_topping::E_topping(std::istream& ist) {
    // The header must have been stripped from the incoming stream at this poin\
t                                                                               
    getline(ist, _name);
